...
void run_test_shfl_up_const_delta_width(const int num, const int blockSize, const int launch_iter, const int shfl_iter) {
  test_shfl_up_const_delta_width<2, 64>(num, blockSize, launch_iter, shfl_iter);
  test_shfl_up_const_delta_width<4, 64>(num, blockSize, launch_iter, shfl_iter);
  test_shfl_up_const_delta_width<8, 64>(num, blockSize, launch_iter, shfl_iter);
  test_shfl_up_const_delta_width<16,64>(num, blockSize, launch_iter, shfl_iter);
  test_shfl_up_const_delta_width<32,64>(num, blockSize, launch_iter, shfl_iter);
  test_shfl_up_const_delta_width<64,64>(num, blockSize, launch_iter, shfl_iter);
}
...
#define LAUNCH_ITER 10
  ...
  for (int i = 1; i <= 1000000; i *= 10) {
    run_test_shfl_up_const_delta_width(64,64,LAUNCH_ITER, i);
  }
